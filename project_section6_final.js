...
function buildAutoPC(budget, usageType) {
    ...
    // --- Budget Allocation (Section 3: JS calculations) ---
    var maxCpuPrice = Math.round(budget * params.cpuBudgetRatio);
    var budgetForCore = Math.round(budget * (1 - params.gpuBudgetRatio));
    var minRamGb = params.minRamGb;

    print("\n  Budget allocation:");
    print("    Max CPU price: $" + maxCpuPrice);
    ...
    var remainingBudget = budget - coreResult.partial_price;
    var accessoryReserve = 200;  // reserve for Case+PSU+Storage+Cooler
    var gpuMaxPrice = remainingBudget - accessoryReserve;

    print("    Remaining budget: $" + remainingBudget);
    print("    GPU max price: $" + gpuMaxPrice);
    ...
    if (gpuMaxPrice > 0) {
        gpu = db.components.find({
            ...
            "specs.score": { $type: "number" }
        }).sort({ "specs.score": -1, price: 1 }).limit(1).toArray()[0];
    }
    ...
    if (!psu) {
        ...
        psu = db.components.find({
            type: "Power Supply",
            price: { $type: "number" }
        }).sort({ "specs.wattage": -1 }).limit(1).toArray()[0];
    }
